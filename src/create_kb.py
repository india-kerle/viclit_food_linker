...
def load_entities(
        entity_path: Path = entity_path) -> Union[Dict[str, str], Dict[str, str]]:
...
def create_dummy_kb(nlp: spacy.language.Language,
    ...
    for qid, desc in desc_dict.items():
        ...
        kb.add_entity(
            entity=qid,
            entity_vector=desc_enc,
            freq=342)  # 342 is an arbitrary value here

    qids = name_dict.keys()
    probs = [1 / (len(qids) + 1)
             for qid in qids]  # to make sure its less than 1
    ...
    cake_terms = [
        ...
        '-cakes',
        'glazed tarts',
        'petrified sponge-cake',
        'toasted oat-cake',
        'nut cakes',
        '-cake']
    for cake_term in cake_terms:
        # sum([probs]) should be <= 1 !
        kb.add_alias(alias=cake_term, entities=qids, probabilities=probs)
