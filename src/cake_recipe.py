...
def make_tasks(nlp: spacy.language.Language,
    ...
    for doc, eg in nlp.pipe(texts, as_tuples=True):
        ...
        for span in spans:
            ...
            if candidates:
                options = [{"id": c.entity_, 'text': id2name_dict.get(
                    c.entity_, c.entity_)} for c in candidates]
                options.append(
                    {"id": "NIL_otherLink", "text": "Not in options"})
                options.append(
                    {"id": "NIL_ambiguous", "text": "Need more context"})
